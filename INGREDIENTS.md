...
- rosetta:tests/bake_mavericks_shim_test.sh: builds an x86_64/10.9 program (`XF`: `-arch x86_64 -mmacosx-version-min=10.9`) with too little header pad, bakes the shim into it so the header grows, and executes the result (`tight.selfcontained`) to prove the grown, baked binary still calls the shim's functions; a precondition miss is a failure, not a skip. On the arm64 release runner that execution is translated by Rosetta, so Rosetta is a required dependency there. The test's central case builds for the host's own arch and needs no Rosetta. Reconsider when an x86_64 host can run it in CI; at the latest, before macOS 28 removes Rosetta.
