...
# wrong. drydock-macho-rewrite tells two kinds of wrong apart (the list below, which
# cli/drydock-macho-rewrite.c's cmd_declassify carried under the heading "FIVE DELIBERATE
...
# whose refusal reaches drydock-macho-rewrite as EX_REFUSED. The mapping's real work is
...
#
# drydock-macho-rewrite gives OUT the INPUT's mode (wa_write_new copies it),
# always a new inode, and refuses an OUT that is IN outright. So this wrapper
# does what the other five do -- drydock-macho-rewrite writes a temp beside the real OUT
...
# have to come before drydock-macho-rewrite runs, because they decide where drydock-macho-rewrite writes. So
...
#
#   * drydock-macho-rewrite's trailing "Wrote <path> (N bytes)" line is DROPPED -- it names the
...
# "Wrote " line -- so neither drydock-macho-rewrite's temp-naming line nor a doubled one can
...
# the two layers below would: drydock-macho-rewrite writes a temp BESIDE OUT and never looks at
...
# `OUT/.OUT.drydock-macho-rewrite-compat.PID` and nothing the caller asked for. The C tool's
...
# is $1, not of the output mw_prepare just named. drydock-macho-rewrite-compat.sh's
...
# installed: drydock-macho-rewrite gave it IN's mode, which is neither of the two answers
