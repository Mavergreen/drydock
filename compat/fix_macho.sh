#!/bin/sh
# fix_macho -- a /bin/sh wrapper around machotool's `lc`, `dylib` and
# `segment`, or one `machotool edit FILE OUT -` when the invocation is worth
# more than one of those (two -rename_seg pairs already are).
#
#   fix_macho <file> [-change old new] [-strip_build_version]
#             [-rename_seg old new] ...
#
# spec: compat/translate.sh holds the grammar -- which verb each flag becomes,
# the statement order, the two capacity caps, and every refusal made before
# any I/O. compat/README.md's "fix_macho: the adopted divergences" table holds
