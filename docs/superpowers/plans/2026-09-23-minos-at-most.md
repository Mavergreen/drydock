...
## For the item-6 comment sweep's held Tasks 7, 8 and 11

Accurate at the commit that last edited this section. Both held tasks start with `git diff --quiet d1cab99 -- FILE || echo CHANGED-STOP` and density counts.

- **Task 7 (`compat/add_version_min.sh`) hits CHANGED-STOP.** This plan edited that file's header (the statement name on line 3; the WHAT THIS REPLACED paragraph deleted; the GRAMMAR, EXIT CODES, STDOUT and STDERR paragraphs and the THIN ONLY comment rewritten, none naming the deleted `mv_add_version_min`) and added one code line after `mw_finish || exit 1`. Measured now: `total=73 comment=48 code=22 first_code=45`. Its Step 1 expectation (`106 / 82 / 21, 77`), its Step 5 `rr` line ranges, its Step 7 after-density (`code=21`; it is `22`) and its commit message's Before/After line must be re-measured. Its replacement header must name `minos if-absent 10.9`, not `version-min set 10.9`.
- **Task 7's Step 5 and commit message claim deletions this plan already made.** The WHAT THIS REPLACED history, the note that the "Added LC_VERSION_MIN_MACOSX" line belonged to the minos verb, and the untested "the two exit codes the COMMAND has of its own are unreachable from here" claim are already gone. Drop Step 5's "deleted, not moved" note and the commit message's "Dropped as history: … / Dropped as an untested can't-happen claim: …" paragraph rather than repeat them.
- **Task 7's new README section already exists.** `## \`add_version_min\`: the differences, and what holds each one` sits directly above `## \`rename_segment\`: exit codes`, where Task 7 planned it. Merge Task 7's rows into it; do not add a second section. Three of its planned rows are wrong: the statement calls no `mv_add_version_min` (deleted); the stderr announcement is `none -> version-min 10.9; sdk 10.9 written`, not `appended LC_VERSION_MIN_MACOSX 10.9`; and the "already present" line is printed by the wrapper (the new code line), not by the core.
- **Task 7's comment edits cite `mv_add_version_min`**, which is deleted. The wrapper-test label "add_version_min: a fat container is refused, untouched, as mv_add_version_min's own mi_open did" keeps the old name so that `compat/README.md`'s "Thin only" table still matches it.
- **Tasks 7 and 8 cite `tests/wrapper_test.sh` by line number** (`:419-424`, `:444`, `:997`, `:1526`). Those lines have moved; find each by the anchor text the task quotes (`hl_case`'s comment is now above `:437`, `hl_case rename_segment __DATA __DATA_HL` is `:455`, the `rsc_u changed` line is `:1643`). The read-only-directory loop above `# 5b.` already asserts "add_version_min: a writable FILE in a read-only directory fails (2), untouched, no temp left" (0a97b93); check it against Task 7's planned case before adding another.
- **Task 8 (`compat/retag_swift_classes.sh`)** is not edited here, so its CHANGED-STOP check passes. Its `1)` arm now also receives `swift-abi set legacy`'s refusal of a chained image. The observable result still matches the C tool, pinned as "retag_swift_classes: a chained Swift binary is the C tool's silent zero, untouched". Task 8's README row "exits 1 (`EX_REFUSED`: not a Mach-O) … skips it silently" must name that case and cite that test.
- **Task 8 must also drop the names of deleted code.** `mswift_retag_file`, `MSWIFT_ERROR` and `MSWIFT_NOT_MACHO` no longer exist. The wrapper names them at `:8`, `:33`, `:41` and `:84` (inside Task 8's `rr $F 2 109`), `:132` (its `rr $F 132 137`) and `:182` (the `1)` arm's `# MSWIFT_NOT_MACHO: the benign skip` comment, which no `rr` covers: reword it). `compat/README.md:212` says the C-tool wording "is still what `mswift_retag_file` itself prints"; that clause is false and must go.
- **Task 11's Deferred bullet is moot.** "`tests/cli_test.sh:1397` (`add_version_min.c` in the present tense) waits for the minos-set plan's `tests/cli_test.sh` work" names nothing: `git grep 'add_version_min\.c' tests/` is empty (control: `git grep add_version_min tests/cli_test.sh` hits). Drop the bullet.
- **Task 9 has landed** (f37f2fa); this plan's change to `mt_tr_add_version_min` is in it.
